	...
	*/
	...
	(function() { 
		...
		http.onerror = function (e) {
			...
			Ti.API.debug(e);
			Ti.API.error('We could not connect to the server.  Please ensure you have a working internet connection and try again.');
			if (error) {
				error(null);
			}
		};

		Ti.API.info('URL('+method+' - ' + server + url + '): ' + url);
		http.open(method, server + url, true);
		http.send(data);
