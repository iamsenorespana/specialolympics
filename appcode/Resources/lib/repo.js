...
module.exports = (function() {
	var PropertyCache = nrequire('/lib/property_cache'), Twitter = nrequire('/lib/twitter'), FbGraph = nrequire('/lib/fb_graph');

	var Cache = {}, cacheHasExpired = function(name) {
	...
	_apiCallFactory = function(cache_name, httpCall) {
		...
		getFacebookNews(function(news) {
			_finishIfCompletedBoth('fb', news);
		});
		getTwitterTimeline(function(news) {
			_finishIfCompletedBoth('twitter', news);
		});
	...
	}), getPhotos = function(finishCall) {

		var flickr = require('ti.flickr');
		flickr.authorize('dc4947a1e6e89c1562d228a972474425', 'ca212de0f1d9f6db');
		...
		var needed_vals = ['donate_url', 'logo_url'], _isDone = function() {
			return needed_vals.reduce(function(b, x) {
				return !!Cache[x] && b;
			}, true);
		...
		if (isIPad)
			needed_vals.push('topbar_message');
		if (_isDone())
			return callback(Cache);
	...
	return {
		cacheHasExpired : cacheHasExpired,
		getNews : getNews,
		getEvents : getEvents,
		getPages : getPages,
		getPhotos : getPhotos,
